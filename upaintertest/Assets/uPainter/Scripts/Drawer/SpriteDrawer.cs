...
{
    class SpriteDrawer : MonoBehaviour
    {
        ...
        protected virtual void Update()
        {
            ...
            if (Physics.Raycast(ray, out hitInfo))
            {
                ...
                if (paintObject is SpritePaintCanvas)
                {
                    if (touchDown)
                    {
                        _drawer.Begin();
                        _drawer.TouchMove(Input.mousePosition, hitInfo.point);
                    }
                    else
                    {
                        _drawer.End();
                        _drawer.HoverMove(Input.mousePosition, hitInfo.point);
                    }
                }
            }
            else
